...
**Vulnerability:** The GET `/iocs/{ioc_id}` endpoint accepted an unbounded integer for the `ioc_id` path parameter. Submitting an extremely large integer (e.g., `999999999999999999999`) caused the underlying SQLAlchemy database driver (like SQLite `pysqlite` or Postgres) to throw a fatal `OverflowError` (int too large to convert to DB INTEGER). This resulted in an unhandled 500 Internal Server Error, leading to a potential Denial of Service (DoS).
**Learning:** Python handles arbitrarily large integers, but relational databases (and their Python drivers) expect fixed-width integers (e.g., 32-bit or 64-bit). Relying on FastAPI's native `int` type hint is insufficient to protect the database layer from overflow exceptions.
**Prevention:** Always strictly constrain path parameters and query parameters that map to database primary keys or integer fields using FastAPI's `Path` or `Query` validation functions (e.g., `ioc_id: int = Path(..., le=2147483647)` for 32-bit signed integers). This ensures invalid payloads are rejected safely with a 422 Unprocessable Entity at the application edge.
