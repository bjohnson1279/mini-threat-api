...
## 2024-10-18 - [Prevent DoS via Rate Limiting on Authentication Endpoint]
**Vulnerability:** The `/auth/token` endpoint was susceptible to Denial of Service (DoS) and brute-force attacks due to the lack of rate limiting. `bcrypt.checkpw()` is a computationally expensive CPU-bound operation. An attacker could rapidly send requests, exhausting CPU resources and starving the application.
**Learning:** Endpoints that perform expensive operations (like password hashing or token generation) must be protected with rate limiting to prevent CPU exhaustion.
**Prevention:** Implement IP-based or user-based rate limiting on sensitive endpoints, specifically authentication routes, to restrict the number of attempts within a given time window.
