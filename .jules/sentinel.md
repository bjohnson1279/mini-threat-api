...
## 2024-10-25 - [Integer Overflow DoS Mitigation in FastAPI Path Parameters]
**Vulnerability:** The `/iocs/{ioc_id}` endpoint used an unbounded integer path parameter (`ioc_id: int`). FastAPI natively parses this to Python's arbitrarily large int type. If a massive integer is provided, passing it to the database driver (e.g., SQLite or Postgres) triggers an internal `OverflowError` or `DataError`, resulting in an unhandled 500 Internal Server Error.
**Learning:** Python's unbounded integers do not automatically match the bounds of underlying database columns (e.g., PostgreSQL `INTEGER` is a signed 32-bit integer). This mismatch allows an attacker to cause application crashes by submitting values exceeding the database capabilities, constituting a Denial of Service risk.
**Prevention:** Always strictly bound path parameters (e.g., using `Path(..., le=2147483647)`) when they correspond to database IDs to ensure FastAPI correctly rejects oversized integers with a `422 Unprocessable Entity` response before the database operation begins.
