...
## 2024-05-19 - FastAPI Response Compression
**Learning:** Returning large lists of JSON objects (like the ones generated by the `/iocs` endpoint) can incur a high network bandwidth and transit time cost. FastAPI does not compress responses by default. However, it provides `GZipMiddleware` out-of-the-box which handles gzip encoding natively when an `Accept-Encoding: gzip` header is present. The `TestClient` handles this gracefully too.
**Action:** When endpoints might return large payloads (such as large lists of database rows serialized to JSON), add `app.add_middleware(GZipMiddleware, minimum_size=1000)` to the application to compress responses that are over `minimum_size` bytes.
